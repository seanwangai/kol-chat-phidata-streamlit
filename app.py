...
class HKStockFilingsDownloader:
    ...
    def get_filings_list(self, stock_id, from_date=None, to_date=None):
        ...
        try:
            ...
            logger.error(f"获取港股公告列表时出错: {str(e)}")
            return []
...
class SECService:
    ...
    @retry_on_failure(max_retries=3)
    def get_filings(self, ticker: str, years: int = 3, forms_to_include: Optional[List[str]] = None) -> List[Document]:
        """获取SEC文件列表"""
        self.rate_limiter.wait_if_needed()
            ...
            for i in range(len(forms)):
                ...
                if form_type in forms_to_include:
                    ...
                    if start_date.date() <= filing_date < end_date.date():  # 不包含结束日期
                        accession_no_no_dashes = accession_numbers[i].replace('-', '')
                        filing_url = f"https://www.sec.gov/Archives/edgar/data/{cik}/{accession_no_no_dashes}/{primary_documents[i]}"
...
class HKStockService:
    ...
    def parse_hk_date(self, date_str: str) -> Optional[datetime.date]:
        """解析港股日期格式"""
        try:
            # 港股日期格式：Release Time:DD/MM/YYYY HH:MM
    ...
    @retry_on_failure(max_retries=3)
    def get_hk_filings(self, ticker: str, years: int = 3, forms_to_include: Optional[List[str]] = None) -> List[Document]:
        """获取港股文件列表"""
        self.rate_limiter.wait_if_needed()
        ...
        try:
            ...
            # 获取公告列表
            all_filings = self.downloader.get_filings_list(stock_id, from_date, to_date)
...
class SECEarningsAnalyzer:
    ...
    def process_document(self, document: Document, processing_prompt: str, model_type: str) -> str:
        ...
        try:
            ...
            回答要求：
            - 開頭以📍这个emoji开頭， 📍後面接這是一份什麼文件，文件目的是什麼，💡第二句換行，直接說結論，回答跟我处理要求有關的結論 都是簡短一句話
...
def main():
    ...
    with st.sidebar:
        ...
        if is_hk_stock(ticker):
            # 港股选项
            use_sec_reports = st.checkbox("季報年報 (Final Results, Quarterly Results, etc.)", value=st.session_state.analyzer_use_sec_reports)
            use_sec_others = st.checkbox("其他公告 (其他类型公告)", value=st.session_state.analyzer_use_sec_others)
            
            use_earnings = st.checkbox("Earnings Call Transcripts", value=st.session_state.analyzer_use_earnings)
            st.caption("财报会议记录")
        else:
            # 美股选项
            use_sec_reports = st.checkbox("季報年報 (10-K, 10-Q, 20-F, etc.)", value=st.session_state.analyzer_use_sec_reports)
            use_sec_others = st.checkbox("其他 (8-K, S-8, etc.)", value=st.session_state.analyzer_use_sec_others)
        ...
        # with col1:
        #     if st.button("🗑️ 清空缓存"):
        #         analyzer.cache_manager.clear()
        #         st.success("缓存已清空")
        
        # with col2:
        #     if st.button("🧹 清理文件"):
        #         analyzer.document_manager.cleanup()
        #         # 重新创建临时目录
        #         analyzer.document_manager = DocumentManager()
        #         st.success("临时文件已清理")
        ...
        # 重置按鈕
        # if st.session_state.get("use_premium_api", False):
        #     if st.button("🔄 重置為免費API"):
        #         st.session_state.use_premium_api = False
        #         st.session_state.premium_access_code = ""
        #         st.success("已重置為免費API服務")
        #         st.rerun()
    ...
    if status.is_processing:
        # 如果正在处理，显示status
        current_step = status.current_status_label or "正在处理..."
        
        with st.expander(f"📋 STATUS", expanded=True):
            st.markdown(f"**{status.current_status_label}**")
            
            if status.total_documents > 0:
                progress_text = f"进度: {status.completed_documents}/{status.total_documents} 个文档"
...
def process_user_question_new(analyzer: SECEarningsAnalyzer, ticker: str, years: int, use_sec_reports: bool, use_sec_others: bool, use_earnings: bool, model_type: str):
    ...
    try:
        ...
        if status.processing_step == 1:
            ...
            status.add_status_message("✅ 用戶問題分析完成")
            status.processing_step = 2
            analyzer.session_manager.update_processing_status(status)
            time.sleep(0.3) # 短暂停留，让用户看到消息
            st.rerun()
        ...
        # 步骤2：获取和筛选文档
            ...
            if selected_forms:
                if is_hk_stock(ticker):
                    ...
                    hk_filings = analyzer.hk_service.get_hk_filings(ticker, years, forms_to_include=hk_forms)
                    ...
                    sec_filings = analyzer.sec_service.get_filings(ticker, years, forms_to_include=selected_forms)
            ...
            if use_earnings:
                ...
                for i, url_path in enumerate(all_earnings_urls):
                    parsed_info = analyzer.earnings_service.parse_transcript_url(url_path)
                    if parsed_info:
                        _ticker, year, quarter = parsed_info
                        status.add_status_message(f"📄 正在处理: {_ticker} {year} Q{quarter} ({i+1}/{len(all_earnings_urls)})")
                        analyzer.session_manager.update_processing_status(status)
                    ...
                    if transcript_info and transcript_info.get('parsed_successfully'):
                        ...
                        logger.warning(f"获取或解析财报失败，跳过: {url_path}")
        ...
        # 步骤3：按日期顺序处理文档
            ...
            if status.completed_documents < len(docs_to_process):
                ...
                # 更新状态
                status.add_status_message(f"正在分析: {current_doc.title}")
                status.update_progress(status.completed_documents, len(docs_to_process), f"📖 分析文档 {status.completed_documents + 1}/{len(docs_to_process)}")
                analyzer.session_manager.update_processing_status(status)
                
                try:
                    # 特殊处理6-K文件
                    if hasattr(current_doc, 'form_type') and current_doc.form_type == '6-K':
                        status.add_status_message(f"检测到6-K文件，开始处理附件")
                        ...
                        status.add_status_message(f"正在下载和处理6-K附件...")
                        analyzer.session_manager.update_processing_status(status)
                        ...
                        status.add_status_message(f"6-K处理完成，生成了 {len(processed_docs)} 个分析文档")
                        
                        # 处理所有6-K相关文档
                        for i, doc in enumerate(processed_docs):
                            status.add_status_message(f"正在分析第 {i+1}/{len(processed_docs)} 个6-K文档: {doc.title}")
                            analyzer.session_manager.update_processing_status(status)
...
\n\n{analysis_result}"""
                            ...
                            status.add_status_message(f"完成第 {i+1} 个6-K文档分析")
                    else:
                        # 普通文档处理
...
\n\n{analysis_result}"""
                    ...
                    status.add_status_message(f"分析失败: {current_doc.title} - {exc}")
                    logger.error(f"文档分析失败: {current_doc.title} - {exc}")
                    
                    # 也将错误信息添加到聊天历史中
                    error_message = f"**⚠️ {current_doc.title} 分析失败:**\n\n{exc}"
                ...
                # 所有文档处理完成
                status.current_status_label = "✅ 所有文档分析完成"
                status.processing_step = 4
                analyzer.session_manager.update_processing_status(status)
                st.rerun()
        ...
        # 步骤4：整合结果
            ...
            status.current_status_label = "📊 正在生成最终报告..."
            status.add_status_message("整合所有分析结果...")
            analyzer.session_manager.update_processing_status(status)
            ...
            status.add_status_message("综合报告生成完毕！")
            status.current_status_label = "✅ 处理完成！"
