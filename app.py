    ...
    try:
        ...
        # 步骤3：按日期顺序处理文档
            ...
            if status.completed_documents < len(docs_to_process):
                ...
                progress_label = f"📖 分析文档 {status.completed_documents + 1}/{len(docs_to_process)}" if language == "中文" else f"📖 Analyzing document {status.completed_documents + 1}/{len(docs_to_process)}"
